...
public class EditHostActivity extends AppCompatActivity implements HostEditorFragment.Listener {
	...
	private long mExistingHostId;
	private MenuItem mSaveHostButton;
	private MenuItem mForgetSavedPasswordButton;
	private SavedPasswordStore mSavedPasswordStore;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mSavedPasswordStore = SavedPasswordStore.get(this);
		...
		groupValues.add(String.valueOf(HostDatabase.HOST_GROUP_NONE));
		for (HostGroupBean group : mHostDb.getHostGroups()) {
			if (group == null || group.getId() <= 0 || group.getName() == null) {
				continue;
			}
			groupNames.add(group.getName());
			groupValues.add(String.valueOf(group.getId()));
		}
		...
		if (fragment == null) {
			fragment = HostEditorFragment.newInstance(
					mHost,
					pubkeyNames,
					pubkeyValues,
					jumpHostNames,
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		mSaveHostButton = menu.findItem(R.id.save);
		mForgetSavedPasswordButton = menu.findItem(R.id.forget_saved_password);

		// If the new host is being created, it can't be added until modifications have been made.
		setAddSaveButtonEnabled(!mIsCreating);
		updateForgetSavedPasswordButton();
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		switch (item.getItemId()) {
			...
			case android.R.id.home:
				attemptSaveAndExit();
				return true;
			case R.id.forget_saved_password:
				forgetSavedPassword();
				return true;
			default:
	...
	private void attemptSaveAndExit() {
		...
		mExistingHostId = mHost.getId();
		if (!mHost.getRememberPassword()) {
			mSavedPasswordStore.clearPassword(mHost.getId());
			clearSavedSsmSecrets(mHost.getId());
		}
	...
	private void forgetSavedPassword() {
		long hostId = getPersistedHostId();
		if (hostId <= 0) {
			return;
		}
		mSavedPasswordStore.clearPassword(hostId);
		clearSavedSsmSecrets(hostId);
		Toast.makeText(this, R.string.hostpref_forget_saved_password_done, Toast.LENGTH_SHORT).show();
		updateForgetSavedPasswordButton();
	}

	private long getPersistedHostId() {
		if (mHost != null && mHost.getId() > 0) {
			return mHost.getId();
		}
		return mExistingHostId;
	}

	private void updateForgetSavedPasswordButton() {
		if (mForgetSavedPasswordButton == null) {
			return;
		}
		long hostId = getPersistedHostId();
		boolean hostAvailable = hostId > 0;
		boolean hasSavedPassword = hostAvailable && (mSavedPasswordStore.hasSavedPassword(hostId)
				|| mSavedPasswordStore.hasScopedSecret(
						SavedPasswordStore.SCOPE_SSM_SECRET_ACCESS_KEY, hostId)
				|| mSavedPasswordStore.hasScopedSecret(
						SavedPasswordStore.SCOPE_SSM_SESSION_TOKEN, hostId));
		mForgetSavedPasswordButton.setVisible(hostAvailable);
		mForgetSavedPasswordButton.setEnabled(hasSavedPassword);
	}

	private void clearSavedSsmSecrets(long hostId) {
		if (hostId <= 0) {
			return;
		}
		mSavedPasswordStore.clearScopedSecret(SavedPasswordStore.SCOPE_SSM_SECRET_ACCESS_KEY, hostId);
		mSavedPasswordStore.clearScopedSecret(SavedPasswordStore.SCOPE_SSM_SESSION_TOKEN, hostId);
	}
