...
public class HostDatabase extends RobustSQLiteOpenHelper implements HostStorage, ColorStorage {
	...
	public final static int DB_VERSION = 30;
	...
	public static final long HOST_GROUP_NONE = -1;
	...
	public static final String CREATE_TABLE_HOST_GROUPS =
			"CREATE TABLE IF NOT EXISTS " + TABLE_HOST_GROUPS
					+ " (_id INTEGER PRIMARY KEY, "
					+ FIELD_HOST_GROUP_NAME + " TEXT UNIQUE COLLATE NOCASE)";

	public static final String CREATE_TABLE_HOST_GROUPS_INDEX =
			"CREATE INDEX IF NOT EXISTS " + TABLE_HOST_GROUPS + FIELD_HOST_GROUP_NAME + "index ON "
					+ TABLE_HOST_GROUPS + " (" + FIELD_HOST_GROUP_NAME + ");";
	...
	static {
		...
		addTableName(TABLE_HOST_GROUPS);
		addIndexName(TABLE_HOST_GROUPS + FIELD_HOST_GROUP_NAME + "index");
	}
	...
	private void createTables(SQLiteDatabase db) {
		...
		db.execSQL(CREATE_TABLE_HOST_GROUPS);
	...
	public void resetDatabase() {
		try {
			...
			mDb.execSQL("DROP TABLE IF EXISTS " + TABLE_HOST_GROUPS);
	...
	public void onRobustUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) throws SQLiteException {
		...
		case 26:
			db.execSQL("ALTER TABLE " + TABLE_HOSTS
					+ " ADD COLUMN " + FIELD_HOST_REMEMBERPASSWORD + " TEXT DEFAULT '" + Boolean.toString(false) + "'");
			// fall through
		case 27:
			db.execSQL("ALTER TABLE " + TABLE_HOSTS
					+ " ADD COLUMN " + FIELD_HOST_JUMPHOSTID + " INTEGER DEFAULT -1");
			// fall through
		case 28:
			db.execSQL(CREATE_TABLE_HOST_GROUPS);
			db.execSQL(CREATE_TABLE_HOST_GROUPS_INDEX);
	...
	public List<HostGroupBean> getHostGroups() {
		List<HostGroupBean> groups;
		Cursor c = mDb.query(
				TABLE_HOST_GROUPS,
				null,
				null,
				...
				null,
				null,
				FIELD_HOST_GROUP_NAME + " COLLATE NOCASE ASC");
		groups = createHostGroupBeans(c);
		c.close();
		return groups;
	}

	public HostGroupBean saveHostGroup(HostGroupBean group) {
		if (group == null) {
			throw new IllegalArgumentException("group is null");
		}

		String normalizedName = normalizeHostGroupName(group.getName());
		...
		try {
			if (groupId <= 0) {
				groupId = mDb.insertWithOnConflict(
						TABLE_HOST_GROUPS, null, group.getValues(), SQLiteDatabase.CONFLICT_IGNORE);
				if (groupId <= 0) {
					HostGroupBean existing = findHostGroupByName(normalizedName);
					if (existing != null) {
						return existing;
					}
					throw new SQLiteException("Unable to save host group");
				}
			} else {
				mDb.update(TABLE_HOST_GROUPS, group.getValues(), "_id = ?",
	...
	public void deleteHostGroup(long groupId) {
		...
		hostValues.put(FIELD_HOST_GROUP_ID, HOST_GROUP_NONE);

		String[] groupIdArg = new String[] {String.valueOf(groupId)};
		mDb.beginTransaction();
		try {
			mDb.update(TABLE_HOSTS, hostValues, FIELD_HOST_GROUP_ID + " = ?", groupIdArg);
			mDb.delete(TABLE_HOST_GROUPS, "_id = ?", groupIdArg);
			mDb.setTransactionSuccessful();
		} finally {
			mDb.endTransaction();
		}
	...
	public HostGroupBean findHostGroupById(long groupId) {
		if (groupId <= 0) {
			return null;
		}
		...
		Cursor c = mDb.query(
				...
				null,
				null);
		HostGroupBean hostGroup = getFirstHostGroupBean(c);
		c.close();
		return hostGroup;
	}

	public HostGroupBean findHostGroupByName(String name) {
		String normalizedName = normalizeHostGroupName(name);
		if (normalizedName == null) {
			return null;
		}
	...
	private List<HostGroupBean> createHostGroupBeans(Cursor c) {
		List<HostGroupBean> groups = new ArrayList<HostGroupBean>();

		final int colId = c.getColumnIndexOrThrow("_id");
		final int colName = c.getColumnIndexOrThrow(FIELD_HOST_GROUP_NAME);
		while (c.moveToNext()) {
			HostGroupBean group = new HostGroupBean();
			group.setId(c.getLong(colId));
			group.setName(c.getString(colName));
			groups.add(group);
		}
	...
	private HostGroupBean getFirstHostGroupBean(Cursor c) {
		HostGroupBean hostGroup = null;

		List<HostGroupBean> groups = createHostGroupBeans(c);
		if (!groups.isEmpty()) {
			hostGroup = groups.get(0);
		}
		return hostGroup;
	}

	private String normalizeHostGroupName(String name) {
		if (name == null) {
			return null;
		}
