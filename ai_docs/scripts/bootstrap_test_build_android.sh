...
echo "==> 2) Generate prompt bundle (existing)"
"$ROOT/ai_docs/scripts/make_prompt_bundle.sh" existing

# Create a test-build bundle that instructs the agent to pick a minimal fix slice and produce a debug APK.
BUNDLES_DIR="$ROOT/references/prompt_bundles"
mkdir -p "$BUNDLES_DIR"
OUT="$BUNDLES_DIR/test_build_android_master.md"
...
echo "✅ Wrote test build bundle: $OUT"
echo
echo "NEXT (one instruction to your IDE agent):"
echo "  Follow instructions in: references/prompt_bundles/test_build_android_master.md"
